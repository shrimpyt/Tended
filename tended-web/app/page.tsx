...
function parseUnit(quantity: string | undefined): string {
  ...
  const match = quantity.match(
    /\b(ml|l|g|kg|oz|lb|fl oz|count|rolls|sheets|pack)\b/i,
  );
  ...
  return (
    ...
    >
      <div className="text-sm font-medium">
        Scanned Barcode: {item.raw_barcode}
      </div>
...
export default function Dashboard() {
  ...
  const { data: inboxItems = [], isLoading: isLoadingInbox } = useQuery<
    InboxItem[],
    Error,
    InboxItem[]
  >({
  ...
  const handleBarcodeScan = async (code: string): Promise<boolean> => {
    try {
      console.log(
        '[Dashboard] Scanned barcode, looking up on OpenFoodFacts:',
        code,
      );
      ...
      console.error('Error in handleBarcodeScan', e);
      return false;
    }
  };
      ...
      <div className="flex-1 p-6 grid grid-cols-[200px_300px_1fr] gap-6 max-w-[1400px] mx-auto w-full">
        ...
        <aside className="border-r border-border pr-4">
          <nav className="flex flex-col gap-2 text-sm font-medium mb-8">
            <div className="px-3 py-2 bg-primary-blue/10 text-primary-blue rounded-md">
              Command Center
            </div>
            <div className="px-3 py-2 text-text-secondary hover:text-text-primary">
              Reports
            </div>
            <div className="px-3 py-2 text-text-secondary hover:text-text-primary">
              Family Settings
          ...
          <Link
            href="/scan"
            className="block w-full bg-primary-blue text-white text-center py-2 rounded-md font-medium text-sm hover:bg-primary-blue/90 transition-colors"
          >
            + Quick Capture
          </Link>
        </aside>
