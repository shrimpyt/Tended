...
export default function ReceiptScanModal({
  ...
  const saveSpendingEntries = async () => {
    ...
    const valid = lineItems.filter(
      li => li.item.trim() && parseFloat(li.amount) > 0,
    );
    if (valid.length === 0) {
      ...
      <div className="glass rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col border border-white/20 shadow-2xl overflow-hidden">
        ...
        {step === 'pick' && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 gap-5">
            <div className="w-16 h-16 rounded-full bg-surface-elevated border border-border flex items-center justify-center text-3xl">
              🧾
            </div>
            <div className="text-center">
              <p className="text-base font-medium mb-1">Import a receipt</p>
              <p className="text-sm text-text-secondary">
                Take a photo or upload from your device to auto-populate
                spending entries.
              </p>
            </div>
            {errorMsg && (
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-primary-blue text-white font-semibold text-sm text-center hover:bg-primary-blue/90 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-blue focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >
              Take Photo
            </button>
            <input
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-surface-elevated border border-border text-foreground font-medium text-sm text-center hover:bg-white/5 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-blue focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >
              Choose from Library
            </button>
            <input
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 gap-4">
            {imagePreview && (
              <Image
                ...
                width={400}
                height={200}
                className="w-full max-h-48 object-contain rounded-xl opacity-50"
                unoptimized
              />
            )}
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              {imagePreview && (
                <Image
                  ...
                  width={400}
                  height={128}
                  className="w-full h-32 object-cover rounded-xl opacity-70"
                  unoptimized
                />
              )}
              {errorMsg && (
                <div className="px-4 py-3 rounded-lg bg-amber/10 border border-amber/30 text-sm text-amber">
                  {errorMsg}
                </div>
              )}
              <div>
                ...
                <p className="text-xs text-text-secondary">
                  Edit names, amounts, or categories before saving.
                </p>
              </div>
              ...
              {lineItems.map((li, index) => (
                <div
                  key={index}
                  className="p-3 rounded-xl bg-surface-elevated border border-border flex flex-col gap-2"
                >
          ...
          <>
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              <div>
                ...
                <p className="text-xs text-text-secondary">
                  These items match your inventory. Toggle to apply.
                </p>
              </div>
              ...
              {restockProposals.map((proposal, idx) => {
                ...
                const newQty = Math.min(
                  item.quantity + proposal.addQuantity,
                  item.max_quantity,
                );
                ...
                return (
                  <div
                    key={idx}
                    className={`p-4 rounded-xl bg-surface-elevated border border-border flex flex-col gap-3 transition-opacity ${!proposal.approved ? 'opacity-40' : ''}`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setRestockProposals(prev =>
                            prev.map((p, i) =>
                              i === idx ? { ...p, approved: !p.approved } : p,
                            ),
                          )
                        }
