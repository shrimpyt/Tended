...
async def run_test():
    ...
    try:
        ...
        # -> Open the Spending tab so the month selector and spending view are visible, then change the month.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div[2]/div[2]/div[2]/div[3]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)

        # -> Click the previous-month arrow to change the displayed month, wait for the view to update, and extract the month label, total spent amount, and entries text for verification.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div[2]/div/div[2]/div/div/div/div/div[2]/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
