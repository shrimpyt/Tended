...
async def run_test():
    ...
    try:
        ...
        # -> Attempt to submit the sign-in form by clicking the visible large button (try clicking element index 54), then wait for the app to respond.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div[2]/div[2]/div[2]/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the Spending tab to open the Spending screen (element index 150).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div[2]/div[2]/div[2]/div[3]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Scan Receipt' control to open the receipt scan modal.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div[2]/div/div[2]/div/div/div/div[2]/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Scan Receipt' control to open the receipt scan modal and wait for the UI to respond.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div[2]/div/div[2]/div/div/div/div/div[2]/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Open the image picker by choosing 'Choose from Library' so an image can be selected (or attempt to open it and observe if the environment blocks file selection).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[7]/div/div[2]/div/div/div/div/div[2]/div[5]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click 'Choose from Library' to open the file picker and attempt to select a receipt image (or observe if the environment blocks file selection).
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Spending Added').first).to_be_visible(timeout=3000)
